...
bool GPS_DATA(void)
{
  ...
  while (SerialGPS.available() > 0)
  {
    ...
    {
      Serial.println("Inside gps.encode GPS.cpp");
...
//        Serial.println(gps.location.lng(), 6);
        ...
        if((sensor.year_gps!=0)&&(sensor.month_gps!=0)&&(sensor.day_gps != 2000))
        {
          ...
          if((sys.pdop_value >= gps.hdop.hdop()) && (gps.hdop.hdop() != 0.0) && (sensor.Fix_Status == 3))
          {
            ...
            Serial.println("GPS.cpp returned true");
            return true;
          }
          else
          {
            ...
            // If fix = 1 then send data without GPS
            if (sys.fix == 1)
            {
              ...
              sys.gps_data_buff[i++] = (sensor.minute_gps)    & 0xFF;
              sys.gps_data_buff[i++] = (sensor.second_gps)    & 0xFF;
              sys.gps_data_buff[i++] = (sensor.fix)           & 0xFF;*/
              
              Serial.printf("No fix available, but uplink being sent\n");
              Serial.printf("sys.fix = %d\n", sys.fix);
              return true;
            } 
